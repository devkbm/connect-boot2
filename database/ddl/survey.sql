create table if not exists COM.GRWSURVEYFORM (
	...
	constraint pk_grwsurveyform primary key(ID)	
) COMMENT = '설문서식';
...
create table if not exists COM.GRWSURVEYITEM (
	...
	constraint pk_grwsurveyitem primary key(ID),
	constraint fk_grwsurveyitem1 foreign key(FORM_ID) references GRWSURVEYFORM(ID)
) COMMENT = '설문항목';